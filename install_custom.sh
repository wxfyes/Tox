...
install_V2bX() {
    ...
    if  [ $# == 0 ] ;then
        ...
        wget --no-check-certificate -N --progress=bar -O /usr/local/V2bX/V2bX-linux.zip https://github.com/wxfyes/Tox/releases/download/${last_version}/Tox-linux-${arch}.zip
        if [[ $? -ne 0 ]]; then
...
EOF
    ...
    # 替换脚本下载地址
    curl -o /usr/bin/V2bX -Ls https://raw.githubusercontent.com/wxfyes/Tox-script/master/tox.sh
