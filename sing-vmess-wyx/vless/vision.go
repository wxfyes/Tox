...
func (c *VisionConn) padding(buffer *buf.Buffer, command byte) *buf.Buffer {
	...
	// Revert to standard V2bX/Xray padding logic
	// range: 900-1400 (V2bX 1.1.10 behavior)
	if contentLen < 900 && c.isTLS {
		nBig, err := rand.Int(rand.Reader, big.NewInt(500))
		if err == nil {
			paddingLen = 900 + int(nBig.Int64()) - contentLen
		} else {
