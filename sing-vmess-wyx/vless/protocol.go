...
func ReadRequest(reader io.Reader) (*Request, error) {
	...
	if isPrivate {
		privateKey := []byte("MOMclashSecureX")
		for i := 0; i < 16; i++ {
			request.UUID[i] ^= privateKey[i%len(privateKey)]
		}
	}
...
func WriteRequest(writer io.Writer, request Request, payload []byte) error {
	...
	uuidBytes := request.UUID
	if request.IsPrivate {
		version = VersionPrivate
		privateKey := []byte("MOMclashSecureX")
		for i := 0; i < 16; i++ {
			uuidBytes[i] ^= privateKey[i%len(privateKey)]
		}
	}
	...
	common.Must(
		...
		common.Error(buffer.Write(uuidBytes[:])),
