...
func NewClient(userId string, flow string, logger logger.Logger) (*Client, error) {
	...
	switch flow {
	case "", "xtls-rprx-vision", "mom-private", "mom-vision":
	default:
...
func (c *PacketConn) getRequest(payload []byte) Request {
	isPrivate := false
	actualFlow := c.flow
	if c.flow == "mom-private" {
		isPrivate = true
		actualFlow = ""
	} else if c.flow == "mom-vision" {
		isPrivate = true
		actualFlow = "xtls-rprx-vision"
	}
	return Request{
...
func (c *PacketConn) Write(b []byte) (n int, err error) {
	if !c.requestWritten {
		...
		if c.requestWritten {
			...
			err = WritePacketRequest(c.Conn, c.getRequest(nil), nil)
			if err == nil {
				n = len(b)
			}
			c.requestWritten = true
...
func (c *PacketConn) WritePacket(buffer *buf.Buffer, destination M.Socksaddr) error {
	...
	if !c.requestWritten {
		...
		if c.requestWritten {
			...
			err := WritePacketRequest(c.Conn, c.getRequest(buffer.Bytes()), buffer.Bytes())
			c.requestWritten = true
			c.access.Unlock()
			return err
		}
