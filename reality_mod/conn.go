	...
	case aead:
		...
		if hc.version == VersionTLS13 {
			...
			if recordType(record[0]) == recordTypeHandshake {
				...
				// Add dynamic noise padding based on payload size
				// Small packets (e.g. Finished): more noise to obscure
				// Large packets (e.g. Certificate): less noise to stay under MTU
				maxAllowedPadding := 1350 - len(record) - c.Overhead()
				if maxAllowedPadding > 0 {
					var randAdd int
					if len(payload) < 200 {
						randAdd = 100 + mrand.Intn(250) // 100-350 for small packets
					} else {
						randAdd = 30 + mrand.Intn(120) // 30-150 for large packets
					}
